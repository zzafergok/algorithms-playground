...
function timSort(arr: number[]): number[] {
  ...
  // Run boyutunu hesaplayan fonksiyon
  ...
  // Insertion sort implementasyonu (küçük run'lar için)
  ...
  // İki sıralı parçayı birleştiren merge fonksiyonu
  const merge = (
    arr: number[],
    left: number,
    mid: number,
  ...
  // Doğal run'ları bulan fonksiyon
  const findRuns = (
    arr: number[]
    ...
    while (i < n - 1) {
      ...
      // Artan veya azalan sıra olup olmadığını kontrol et
      if (arr[i] > arr[i + 1]) {
        descending = true;
